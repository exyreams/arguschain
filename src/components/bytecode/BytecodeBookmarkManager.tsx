...
interface BytecodeBookmarkManagerProps {
  onLoadBookmark: (bookmark: BytecodeAnalysisBookmark) => void;
  className?: string;
}

export function BytecodeBookmarkManager({
  onLoadBookmark,
  className = "",
}: BytecodeBookmarkManagerProps) {
  ...
  const filteredBookmarks = bookmarks.filter(
    (bookmark) =>
      ...
      bookmark.query_config.contractAddresses?.some((addr: string) =>
  ...
  const handleQuickSave = async () => {
    ...
    // Extract contract addresses from URL or current state
    const contractMatch = currentUrl.match(/\/bytecode-analysis\/([^/?]+)/);
    const transactionMatch = currentUrl.match(/\/transaction\/([^/?]+)/);

    if (!contractMatch && !transactionMatch) {
      toast.error("No contract or transaction found in current URL");
      return;
    }

    const network = urlParams.get("network") || "mainnet";
    const analysisType = transactionMatch ? "transaction" : "contracts";

    try {
      const bookmarkData = {
        title:
          analysisType === "transaction"
            ? `Transaction Analysis ${transactionMatch?.[1]?.slice(0, 8)}...`
            : `Contract Analysis ${contractMatch?.[1]?.slice(0, 8)}...`,
        description: `Bytecode analysis for ${analysisType} on ${network}`,
        bookmark_type: "bytecode_analysis" as const,
        query_config: {
          contractAddresses: contractMatch ? [contractMatch[1]] : [],
          contractNames: [],
          network,
          analysisType: analysisType as "contracts" | "transaction",
          transactionHash: transactionMatch?.[1],
          includeInternalTransactions: true,
          includePyusdAnalysis: true,
          includeGasAnalysis: true,
          includeFunctionCategories: true,
          lastAnalysisResults: {
            totalContracts: contractMatch ? 1 : 0,
            totalTransactions: transactionMatch ? 1 : 0,
            totalGasUsed: 0,
            pyusdInteractions: 0,
            failedTraces: 0,
          },
  ...
  const getBookmarkDisplayText = (bookmark: BytecodeAnalysisBookmark) => {
    if (
      bookmark.query_config.analysisType === "transaction" &&
      bookmark.query_config.transactionHash
    ) {
      return `TX: ${shortenAddress(bookmark.query_config.transactionHash, 6)}`;
    }

    if (
      bookmark.query_config.contractAddresses &&
      bookmark.query_config.contractAddresses.length === 1
    ) {
      return shortenAddress(bookmark.query_config.contractAddresses[0]);
    }

    return `${bookmark.query_config.contractAddresses?.length || 0} contracts`;
  };
                    ...
                    <div className="flex items-start justify-between mb-2">
                      ...
                      >
                        ...
                        <p className="text-xs text-[#8b9dc3] font-mono">
                          {getBookmarkDisplayText(bookmark)} •{" "}
                          {bookmark.query_config.network}
                        </p>
