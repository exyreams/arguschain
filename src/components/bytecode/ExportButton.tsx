...
export function ExportButton({
      ...
      switch (format) {
        ...
        case "report":
          if (contractAddress && data) {
            const report = generateSummaryReport(contractAddress, data);
        ...
        case "share":
          if (contractAddress && data) {
            const summary = {
              ...
              bytecodeSize: data.bytecodeSize || 0,
              complexity: data.complexity || 0,
              patterns: data.patterns || 0,
              comparisonMode,
              compareWith: compareWith.length,
            };
            const shareUrl = generateShareableURL(contractAddress, summary);
        ...
        case "bookmark":
          if (contractAddress && data) {
            ...
            saveBookmark(
              ...
              analysisType,
              summary,
              `Bytecode Analysis (${comparisonMode}) - ${contractAddress.slice(0, 10)}...`
            );
  ...
  const generateSummaryReport = (contractAddress: string, data: any) => {
    return {
      contractAddress,
      network,
      analysisType: `Bytecode Analysis (${comparisonMode})`,
      ...
      optimization: data.optimizationSuggestions || [],
      comparison:
        comparisonMode === "comparison" ? data.comparisonResults || [] : null,
    };
  };

  const generateShareableURL = (contractAddress: string, summary: any) => {
    const baseUrl = window.location.origin;
    const params = new URLSearchParams({
      contract: contractAddress,
      network,
      mode: summary.comparisonMode,
      functions: summary.functionCount.toString(),
      size: summary.bytecodeSize.toString(),
      complexity: summary.complexity.toString(),
      patterns: summary.patterns.toString(),
      comparisons: summary.compareWith.toString(),
    });
    return `${baseUrl}/bytecode-analysis?${params.toString()}`;
  };
  ...
  const saveBookmark = (
    ...
    const bookmark = {
      id: `${contractAddress}_${summary.comparisonMode}`,
      title,
      analysisType,
      contractAddress,
      network,
