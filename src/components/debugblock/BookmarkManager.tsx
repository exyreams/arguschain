...
import {
  useDebugBlockBookmarks,
  useCreateDebugBlockBookmark,
  useDeleteDebugBlockBookmark,
} from "@/hooks/debugblock/useDebugBlockBookmarks";
import type { DebugBlockBookmark } from "@/lib/debugblock/bookmarks";
...
export function BookmarkManager({
  ...
  const { data: bookmarks = [], isLoading } = useDebugBlockBookmarks();
  const createBookmark = useCreateDebugBlockBookmark();
  const deleteBookmark = useDeleteDebugBlockBookmark();
  ...
  const handleLoadBookmark = (bookmark: DebugBlockBookmark) => {
    onLoadBookmark(
      bookmark.query_config.blockIdentifier,
  ...
  const handleQuickSave = async () => {
    ...
    const blockMatch = currentUrl.match(/\/debug-block-trace\/([^/?]+)/);
    ...
    try {
      const bookmarkData = {
        title: `Debug Block ${blockIdentifier}`,
