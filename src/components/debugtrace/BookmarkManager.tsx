...
import {
  useDebugTraceBookmarks,
  useCreateDebugTraceBookmark,
  useUpdateDebugTraceBookmark,
  useDeleteDebugTraceBookmark,
  useDebugTraceBookmarkByTxHash,
} from "@/hooks/debugtrace/useDebugTraceBookmarks";
import type { DebugTraceBookmark } from "@/lib/debugtrace/bookmarks";
...
interface BookmarkManagerProps {
  ...
  onLoadBookmark?: (bookmark: DebugTraceBookmark) => void;
  onSignUpClick?: () => void;
  className?: string;
}
...
export function BookmarkManager({
  ...
  const { data: bookmarks, isLoading } = useDebugTraceBookmarks();
  const { data: existingBookmark } = useDebugTraceBookmarkByTxHash(
    txHash || "",
    network
  );
  const createBookmark = useCreateDebugTraceBookmark();
  const updateBookmark = useUpdateDebugTraceBookmark();
  const deleteBookmark = useDeleteDebugTraceBookmark();
  ...
  const handleSaveBookmark = async () => {
    ...
    try {
      await createBookmark.mutateAsync({
        title,
        description: `Debug trace analysis for transaction on ${network}`,
        tx_hash: txHash,
        network,
  ...
  const handleEditBookmark = (bookmark: DebugTraceBookmark) => {
    setEditingBookmark({
      id: bookmark.id,
  ...
  const handleLoadBookmark = (bookmark: DebugTraceBookmark) => {
    onLoadBookmark?.(bookmark);
    setIsOpen(false);
  };

  const getAnalysisTypeLabel = (config: DebugTraceBookmark["query_config"]) => {
    switch (config.trace_method) {
      case "structLog":
  ...
  const getAnalysisTypeColor = (config: DebugTraceBookmark["query_config"]) => {
    switch (config.trace_method) {
      case "structLog":
  ...
  const filteredBookmarks =
    bookmarks?.filter((bookmark) => {
      ...
      const matchesFilter =
        filterType === "all" ||
        bookmark.query_config.trace_method === filterType;
  ...
  return (
    <div className={`relative z-50 ${className}`}>
      <div className="flex items-center gap-2">
        ...
        {txHash && analysisConfig && !existingBookmark && (
          <Button
                ...
                >
                  <Dropdown
                    ...
                    options={[
                      ...
                      { value: "both", label: "Unified Analysis" },
                      { value: "callTracer", label: "Contract Analysis" },
                      { value: "structLog", label: "Opcode Analysis" },
                    ]}
                  />
                </div>
              </div>
                      ...
                      >
                        {editingBookmark?.id === bookmark.id ? (
                          ...
                          <>
                            ...
                            {bookmark.analysis_results && (
                              <div className="grid grid-cols-2 gap-2 text-xs">
                                {bookmark.analysis_results.gas_used && (
                                  <div className="flex items-center gap-1">
                                    <BarChart3 className="h-3 w-3 text-[#00bfff]" />
                                ...
                                {bookmark.analysis_results.call_count && (
                                  <div className="flex items-center gap-1">
                                ...
                                {bookmark.analysis_results.opcode_count && (
                                  <div className="flex items-center gap-1">
                                    <span className="text-[#8b9dc3]">
                                      Opcodes:
                                    </span>
                                    <span className="text-white">
                                      {bookmark.analysis_results.opcode_count.toLocaleString()}
                                    </span>
                                  </div>
                                )}

                                {bookmark.analysis_results.success_rate !==
                                  undefined && (
                                  <div className="flex items-center gap-1">
                                    <span className="text-[#8b9dc3]">
                                      Success:
                                    ...
                                    <span
                                      className={`${bookmark.analysis_results.success_rate >= 90 ? "text-[#10b981]" : "text-[#f59e0b]"}`}
                                    >
                                      {bookmark.analysis_results.success_rate.toFixed(
                                        1
                                      )}
                                      %
                                    </span>
