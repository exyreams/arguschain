...
export function ExportButton({
        ...
        case "share":
          if (txHash && data) {
            const summary = {
              ...
              totalOpcodes: data.totalOpcodes || 0,
              analysisType,
            };
        ...
        case "bookmark":
          if (txHash && data) {
            ...
            saveBookmark(
              ...
              analysisType,
              summary,
              `${analysisType} Analysis of ${txHash.slice(0, 10)}...`
            );
  ...
  const exportChartAsImage = async (
    ...
    // Find the chart container with multiple fallbacks for debug trace charts
    const chartElement =
      ...
      (element.querySelector(".debug-trace-chart") as HTMLElement) ||
      element;
  ...
  const generateSummaryReport = (txHash: string, data: any) => {
    return {
      ...
      analysisType: `Debug Trace Analysis (${analysisType})`,
      timestamp: new Date().toISOString(),
      summary: {
        totalGas: data.totalGas || 0,
        totalOpcodes: data.totalOpcodes || 0,
        executionSteps: data.executionSteps || 0,
        memoryOperations: data.memoryOperations || 0,
        storageOperations: data.storageOperations || 0,
      },
      performance: {
        gasEfficiency: data.gasEfficiency || "Unknown",
  ...
  const generateShareableURL = (txHash: string, summary: any) => {
    ...
    const params = new URLSearchParams({
      tx: txHash,
      type: analysisType || "unified",
      gas: summary.totalGas.toString(),
      opcodes: summary.totalOpcodes.toString(),
    });
    return `${baseUrl}/debug-trace/${txHash}?${params.toString()}`;
  };
