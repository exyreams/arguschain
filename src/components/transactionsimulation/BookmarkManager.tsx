...
import {
  useTransactionSimulationBookmarks,
  useCreateTransactionSimulationBookmark,
  useUpdateTransactionSimulationBookmark,
  useDeleteTransactionSimulationBookmark,
  useTransactionSimulationBookmarkByQuery,
} from "@/hooks/transactionsimulation/useTransactionSimulationBookmarks";
import type { TransactionSimulationBookmark } from "@/lib/transactionsimulation/bookmarks";

interface SimulationQueryConfig {
  network: string;
  fromAddress: string;
...
interface BookmarkManagerProps {
  queryParams?: SimulationQueryConfig;
  analysisResults?: {
    gasUsed?: number;
    gasPrice?: number;
    success?: boolean;
  ...
  onLoadBookmark?: (bookmark: TransactionSimulationBookmark) => void;
  onSignUpClick?: () => void;
  className?: string;
}
...
export function BookmarkManager({
  ...
  const { data: bookmarks, isLoading } = useTransactionSimulationBookmarks();
  const { data: existingBookmark } = useTransactionSimulationBookmarkByQuery(
    queryParams?.network || "",
    queryParams?.fromAddress || "",
    queryParams?.simulationType || "single",
    queryParams?.functionName
  );
  const createBookmark = useCreateTransactionSimulationBookmark();
  const updateBookmark = useUpdateTransactionSimulationBookmark();
  const deleteBookmark = useDeleteTransactionSimulationBookmark();
  ...
  const handleSaveBookmark = async () => {
    ...
    if (!queryParams || !queryParams.network || !queryParams.fromAddress) {
      return;
    }

    const title = `${queryParams.simulationType} simulation - ${queryParams.functionName || "Unknown"} (${queryParams.network})`;

    try {
      const bookmarkData = {
        title,
        description: `${queryParams.simulationType} simulation on ${queryParams.network} from ${queryParams.fromAddress.slice(0, 8)}...`,
        bookmark_type: "transactions_simulation" as const,
        query_config: {
          ...queryParams,
          // Store analysis results within query_config if needed
          ...(analysisResults && { lastAnalysisResults: analysisResults }),
        },
      };
  ...
  const handleEditBookmark = (bookmark: TransactionSimulationBookmark) => {
    setEditingBookmark({
      id: bookmark.id,
  ...
  const handleLoadBookmark = (bookmark: TransactionSimulationBookmark) => {
    onLoadBookmark?.(bookmark);
    setIsOpen(false);
  };

  const getSimulationTypeLabel = (simulationType: string) => {
  ...
  const getSimulationTypeColor = (simulationType: string) => {
    switch (simulationType) {
      case "single":
  ...
  const availableTypes = Array.from(
    new Set(
      (Array.isArray(bookmarks) ? bookmarks : [])
        ?.map((b) => b.query_config?.simulationType)
        .filter(
          (type): type is "single" | "batch" | "comparison" =>
            Boolean(type) &&
            ["single", "batch", "comparison"].includes(type as string)
        ) || []
    )
  );
  ...
  const filteredBookmarks =
    (Array.isArray(bookmarks) ? bookmarks : [])?.filter((bookmark) => {
      const matchesSearch =
        ...
        bookmark.query_config?.functionName
          ?.toLowerCase()
      ...
      const matchesTypeFilter =
        filterType === "all" ||
        bookmark.query_config?.simulationType === filterType;
  ...
  return (
    <div className={`relative z-50 ${className}`}>
      <div className="flex items-center gap-2">
        {/* Save Current Simulation Button */}
        {queryParams &&
          queryParams.network &&
          queryParams.fromAddress &&
          !existingBookmark && (
            ...
            >
              ...
              {createBookmark.isPending ? "Saving..." : "Save Simulation"}
            </Button>
          )}
                    ...
                    {editingBookmark?.id === bookmark.id ? (
                      <div className="space-y-3">
                        <div>
                          <Input
                            ...
                            className="w-full bg-[rgba(15,20,25,0.5)] border border-[rgba(0,191,255,0.2)] rounded px-2 py-1 text-sm text-accent-primary"
                      ...
                      <>
                          ...
                          >
                            ...
                            <p className="text-xs text-[#8b9dc3] font-mono">
                              {bookmark.query_config?.functionName ||
                                "Unknown function"}{" "}
                              •{" "}
                              {bookmark.query_config?.fromAddress?.slice(0, 8)}
                              ...
                            </p>
                            {bookmark.description && (
                        ...
                        <div className="flex items-center gap-2 mb-2">
                          <span
                            className={`px-2 py-1 rounded text-xs font-medium ${getSimulationTypeColor(
                              bookmark.query_config?.simulationType || "single"
                            )}`}
                          >
                          ...
                          <div className="flex items-center gap-1 text-xs text-[#6b7280]">
                            ...
                            {bookmark.query_config?.lastAnalysisResults?.gasUsed?.toLocaleString() ||
                              0}{" "}
                            gas
                          </div>
                          <div className="flex items-center gap-1 text-xs text-[#6b7280]">
